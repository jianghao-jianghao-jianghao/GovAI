...
const LoadingScreen = () => (
  <div className="min-h-screen bg-gradient-to-br from-slate-100 to-slate-200 flex items-center justify-center">
    <div className="flex flex-col items-center">
      <div className="w-16 h-16 bg-blue-600 rounded-xl flex items-center justify-center text-white font-bold text-3xl shadow-lg mb-4">
        G
      </div>
...
const App = () => {
  ...
  const toast = {
    success: (t) => addToast(t, "success"),
    error: (t) => addToast(t, "error"),
  ...
  const NavItem = ({ id, icon: Icon, label, perm }) => {
    ...
    return (
      <button
        ...
        className={`w-full flex items-center ${sidebarCollapsed ? "justify-center px-2" : "space-x-3 px-4"} py-3 text-sm font-medium transition-all border-l-4 ${activeTab === id ? "bg-blue-900/50 text-white border-blue-500" : "border-transparent text-gray-400 hover:bg-gray-800 hover:text-white"}`}
  ...
  return (
    <ToastContext.Provider value={toast}>
      <div className="flex h-screen bg-[#f3f4f6] text-gray-900 font-sans">
        <div
          className={`${sidebarCollapsed ? "w-20" : "w-64"} bg-[#111827] text-gray-300 flex flex-col shrink-0 shadow-2xl z-20 transition-all duration-300 ease-in-out`}
        >
          <div className="h-16 flex items-center justify-between px-4 border-b border-gray-800 bg-[#0f172a]">
            {!sidebarCollapsed && (
              <div className="flex items-center group cursor-pointer overflow-hidden whitespace-nowrap">
                <div className="w-8 h-8 bg-blue-600 rounded-lg flex items-center justify-center text-white font-bold shadow-lg mr-3 group-hover:rotate-12 transition-transform duration-300">
                  G
                </div>
                <span className="text-white font-bold text-lg group-hover:text-blue-400 transition-colors duration-300">
                  GovAI 智政
                </span>
              </div>
            )}
          ...
          <div className="flex-1 py-6 space-y-1 overflow-y-auto overflow-x-hidden">
            {!sidebarCollapsed && (
              <div className="px-6 mb-2 text-[10px] font-bold text-gray-500 uppercase tracking-wider animate-in fade-in duration-300">
                办公作业
              </div>
            )}
            ...
            <NavItem
              ...
              icon={FileText}
              label="智能公文"
              perm={PERMISSIONS.APP_DOC_WRITE}
            />
            <NavItem
              id="chat"
              icon={MessageSquare}
              label="智能问答"
              perm={PERMISSIONS.APP_QA_CHAT}
            />

            {!sidebarCollapsed && (
              <div className="px-6 mt-6 mb-2 text-[10px] font-bold text-gray-500 uppercase tracking-wider animate-in fade-in duration-300">
                资源与管理
              </div>
            )}
            {sidebarCollapsed && (
              <div className="h-4 border-t border-gray-800 mx-4 my-2"></div>
            )}
            <NavItem
              id="users"
              icon={Users}
              label="用户权限"
              perm={PERMISSIONS.SYS_USER_MGMT}
            />
            <NavItem
              id="kb"
              icon={Database}
              label="知识库"
              perm={PERMISSIONS.RES_KB_MGMT}
            />
            <NavItem
              id="graph"
              icon={Network}
          ...
          <div className="p-4 bg-[#0f172a] border-t border-gray-800 relative">
                ...
                <div className="absolute bottom-full left-2 right-2 mb-3 bg-slate-800 border border-slate-700 rounded-xl shadow-2xl overflow-hidden z-10 w-60 animate-in slide-in-from-bottom-2 fade-in duration-200">
                  <div className="p-4 border-b border-slate-700 bg-slate-800/50">
                    ...
                    <div className="mt-2 flex items-center text-xs text-blue-400 bg-blue-900/20 px-2 py-1 rounded w-fit">
                      <Shield size={10} className="mr-1" /> {user.department} -{" "}
                      {user.roleName || "User"}
                    </div>
                  </div>
                  <div className="p-1">
                    ...
                    <button
                      ...
                      className="w-full text-left px-3 py-2 text-sm text-red-400 hover:bg-slate-700 hover:text-red-300 rounded-lg flex items-center transition-colors"
                    >
            ...
            >
              ...
              {!sidebarCollapsed && (
                <MoreHorizontal size={16} className="text-gray-500 ml-2" />
              )}
            </div>
          </div>
        </div>
        <div className="flex-1 flex flex-col min-w-0 h-screen">
          <div className="h-14 bg-white border-b border-gray-200 flex justify-between items-center px-6 shadow-sm z-10">
            ...
            <div className="flex items-center space-x-2 text-xs text-gray-400">
              <Activity size={12} className="text-green-500" /> 系统正常
            </div>
          </div>
