...
import { apiGetGraphNodes, apiGetGraphEdges } from "../api";
...
export const GraphView = ({ toast, focusNodeId }) => {
  ...
  useEffect(() => {
    ...
    const FALLBACK_LINKS = [
      { source: "数据安全", target: "分类分级" },
      { source: "数据安全", target: "数据安全法" },
      { source: "数据安全", target: "个人信息保护" },
      { source: "数据安全", target: "风险评估" },
      { source: "数字政府", target: "以人民为中心" },
      { source: "数字政府", target: "系统观念" },
      { source: "数字政府", target: "云平台" },
      { source: "数字政府", target: "大数据" },
      { source: "数字政府", target: "一网通办" },
      { source: "分类分级", target: "数据安全法" },
      { source: "分类分级", target: "国家安全" },
      { source: "国家安全", target: "数据安全" },
      { source: "应急响应", target: "数据安全" },
      { source: "人工智能", target: "大数据" },
      { source: "人工智能", target: "数字政府" },
      { source: "一网通办", target: "电子证照" },
      { source: "一网通办", target: "跨省通办" },
    ];

    const initSim = (nodeData, linkData) => {
      sim.nodes = nodeData.map((n) => ({
        ...n,
    ...
    (async () => {
      try {
        ...
        if (apiNodes.length > 0) {
          ...
          let gIdx = 1;
          const nodeData = apiNodes.map((n) => {
            if (!groupMap[n.entity_type]) groupMap[n.entity_type] = gIdx++;
            return {
              id: n.name,
              group: groupMap[n.entity_type],
              val: Math.max(10, n.weight || 15),
            };
          });
          const linkData = apiEdges.map((e) => ({
            source: e.source_name,
            target: e.target_name,
          }));
          initSim(nodeData, linkData);
          return;
        }
      } catch {
        /* 后端不可用时使用演示数据 */
      }
      initSim(FALLBACK_NODES, FALLBACK_LINKS);
    })();
  }, []);
    ...
    const resize = () => {
      if (containerRef.current) {
        ...
        if (sim.camera.x === 0 && sim.camera.y === 0) {
          sim.camera.x = w / 2;
          sim.camera.y = h / 2;
          sim.camera.targetX = w / 2;
          sim.camera.targetY = h / 2;
        }
      }
    };
    ...
    const render = (time) => {
      ...
      for (let i = 0; i < sim.nodes.length; i++) {
        for (let j = i + 1; j < sim.nodes.length; j++) {
          ...
          const fx = (dx / dist) * f;
          const fy = (dy / dist) * f;
          if (!n1.fixed && n1 !== sim.dragging) {
            n1.vx += fx;
            n1.vy += fy;
          }
          if (!n2.fixed && n2 !== sim.dragging) {
            n2.vx -= fx;
            n2.vy -= fy;
          }
        }
      ...
      ctx.setTransform(
        sim.camera.k * (window.devicePixelRatio || 1),
        0,
        0,
        sim.camera.k * (window.devicePixelRatio || 1),
        sim.camera.x * (window.devicePixelRatio || 1),
        sim.camera.y * (window.devicePixelRatio || 1),
      );
      ...
      for (
        let x = Math.floor(viewL / gridSize) * gridSize;
        x < viewR;
        x += gridSize
      ) {
        ctx.moveTo(x, viewT);
      ...
      for (
        let y = Math.floor(viewT / gridSize) * gridSize;
        y < viewB;
        y += gridSize
      ) {
        ctx.moveTo(viewL, y);
      ...
      for (const link of sim.links) {
        const isHighlight =
          sim.selected &&
          (sim.selected === link.source || sim.selected === link.target);
        ctx.beginPath();
        ...
        if (true) {
          const offset =
            ((time / 1000) * 60 + (link.source.x + link.target.y)) % 100;
          const t = offset / 100;
  ...
  return (
    <div
      ref={containerRef}
      className="h-full w-full bg-[#020617] relative overflow-hidden flex shadow-inner"
    >
      <canvas
        ref={canvasRef}
      ...
      />
      <div
        className={`absolute top-4 right-4 w-80 bg-slate-900/90 backdrop-blur border border-slate-700 text-slate-200 p-6 rounded-xl shadow-2xl transition-all duration-300 transform ${selectedNodeId ? "translate-x-0 opacity-100" : "translate-x-[120%] opacity-0"}`}
      >
        <div className="flex justify-between items-start mb-4">
          <h3 className="text-xl font-bold text-white flex items-center">
            <Share2 size={20} className="mr-2 text-blue-400" />
            {selectedNodeId}
          </h3>
          <button
            onClick={() => {
        ...
        <div className="space-y-4">
          <div className="p-3 bg-slate-800 rounded border border-slate-700">
            ...
            <div className="text-xs text-gray-400 space-y-1">
              <div className="flex items-center">
                <MousePointer2 size={12} className="mr-2" /> 拖拽节点可固定位置
                (红色描边)
              </div>
              <div className="flex items-center">
          ...
          <div className="pt-2 border-t border-slate-700">
            {sim.selected && sim.selected.fixed ? (
              <button
                onClick={() => {
                  sim.selected.fixed = false;
                }}
                className="w-full py-2 bg-slate-700 hover:bg-slate-600 text-white rounded font-bold text-sm transition-all flex items-center justify-center"
              >
                <Zap size={14} className="mr-2 text-yellow-400" /> 解除固定
                (Unpin)
              </button>
            ) : (
      ...
      <div className="absolute top-4 left-4 flex space-x-2 pointer-events-none">
        <div className="bg-slate-900/80 backdrop-blur border border-slate-700 text-cyan-400 text-xs px-3 py-1 rounded font-mono flex items-center">
          <Activity size={12} className="mr-2 animate-pulse" />
          LIVE
        </div>
        <div className="bg-slate-900/80 backdrop-blur border border-slate-700 text-purple-400 text-xs px-3 py-1 rounded font-mono">
          SIM: ACTIVE
        </div>
        <div className="bg-slate-900/80 backdrop-blur border border-slate-700 text-emerald-400 text-xs px-3 py-1 rounded font-mono flex items-center">
          <Navigation size={12} className="mr-2" />
          CAM: {sim.camera.k.toFixed(2)}x
        </div>
      </div>
    </div>
  );
