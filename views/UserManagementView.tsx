...
const RoleEditor = ({
  ...
  const togglePerm = (key: string) => {
    ...
    setFormData((prev: any) => {
      const s = new Set(prev.permissions);
      if (s.has(key)) s.delete(key);
      else s.add(key);
      return { ...prev, permissions: Array.from(s) };
    });
  };
  ...
  const setScopeAll = (
    ...
    if (isSystem) return;
    setFormData((prev: any) => {
      let newPerms = prev.permissions.filter(
